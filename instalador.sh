	...
	then
		...
		then
			...
			return 0
		      ...
		      echo >&2
        	      echo >&2
                      echo "Profile path do Firefox não encontrado" >&2
		      echo "Informe o path utilizando a opção -p <profile-path>" >&2
		      echo "Consulte o profile path em about:profiles" >&2
		      return 1
		fi
    fi
}
...
ExtractCShell()
{
    ...
    bunzip2 -c payload.bz2 | tar xf - -C ${INSTALL_DIR}/tmp > /dev/null 2>&1
